...
This version adds *tunable* weight adjustments:
...
class MorphoTokenizer:
    ...
    def _score_piece(self, p: str) -> Piece:
        ...
        if p in self.root_weight:
            ...
            #     This is where 'thyroid', 'second', etc. get an extra push.
            if 4 <= L <= 9:
                score += self.MID_ROOT_ADJUST

            # (3) Very long roots: penalize or reward beyond length threshold
            #     (e.g., discourage accepting 'biomedical' as a single root if LONG_ROOT_ADJUST < 0).
